...
class Environment(gym.Env):
    def __init__(self, grid_size=(20, 20), num_dynamic_obstacles=3, num_rays=8, max_ray_length=3,
        ...
        # 난이도 관련 변수 수정
        self.max_difficulty = 4  # 0부터 시작하므로 총 5단계
        ...
        self.difficulty_settings = {
            0: {"distance_range": (3, 5),   "num_obstacles": 5},  # 시작: 5개
            1: {"distance_range": (4, 7),   "num_obstacles": 7},
            2: {"distance_range": (5, 9),   "num_obstacles": 9},
            3: {"distance_range": (6, 11),  "num_obstacles": 11},
            4: {"distance_range": (7, 13),  "num_obstacles": 13}  # 최종: 13개
        }
    ...
    def step(self, action):
        ...
        if done:
            ...
            # 성공 시 연속 성공 횟수 증가, 실패 시 리셋
            if tuple(self.robot_pos.astype(int)) == self.goal:
                self.consecutive_successes += 1
                if self.consecutive_successes >= self.consecutive_successes_required:
    ...
    def reset(self):
        ...
        # 난이도에 따른 장애물 수 설정
        self.num_dynamic_obstacles = self.difficulty_settings[self.current_difficulty]["num_obstacles"]
    ...
    def get_goal_position(self):
        ...
        # 현재 난이도에 따른 거리 범위 가져오기
        min_distance, max_distance = self.difficulty_settings[self.current_difficulty]["distance_range"]
    ...
    def compute_reward(self, collision):
        ...
        for i, ray in enumerate(rays):
            ...
            if distance < self.max_ray_length:
                if distance < 1.0:
                    penalty = -1 * direction_weights[i]  # -1.0 → -0.5로 감소
                elif 2.0 < distance < 4.0:
                    penalty = 0.05 * direction_weights[i]  # 0.05 → 0.02로 감소
                else:
        ...
        if angle_diff < np.pi/4:  # 전방 45도 이내
            ...
            direction_reward = -1.0  # 잘못된 방향 페널티
    ...
    def update_difficulty(self, reward):
        """이 메서드는 더 이상 사용하지 않음"""
        pass
    ...
    def increase_difficulty(self):
        if self.current_difficulty < self.max_difficulty:
            ...
            print(f"Current Difficulty: {self.current_difficulty + 1}/5")
            print(f"Consecutive Successes: {self.consecutive_successes}")
            print(f"New Settings:")
            print(f"  - Distance Range: {self.difficulty_settings[self.current_difficulty + 1]['distance_range']}")
            print(f"  - Number of Obstacles: {self.difficulty_settings[self.current_difficulty + 1]['num_obstacles']}")
            print("==================================================\n")
            
            self.current_difficulty += 1
            self.consecutive_successes = 0  # 난이도 상승 후 연속 성공 횟수 리셋
