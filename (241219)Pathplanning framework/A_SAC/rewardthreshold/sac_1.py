...
def main():
    ...
    env = Environment(
        num_rays=8,
        initial_threshold=50,      # 초기 임계값 낮춤
        threshold_increment=30,    # 증가량 조정 (30씩 증가하다가 마지막에는 10만)
        window_size=25            # optuna에서 찾은 최적값 사용
    )
    ...
    model = SAC(
        "MlpPolicy", 
        env,
        learning_rate=0.0024251124513035398,
        buffer_size=10000,
        learning_starts=1000,
        batch_size=224,
        tau=0.03691504005243593,
        gamma=0.9262384922982472,
        train_freq=2,
        gradient_steps=4,
        ent_coef="auto",
