...
class Planner_GCS(DEVSAtomicModel):
    ...
    def funcExternalTransition(self, strPort, objEvent):
        if strPort == "MyManeuverState":
            ...
            try:
                self.assign_goal(agent_id, self.agents_goals[agent_id].pop(0))
            except:
                print('goal empty')
            self.setStateValue('mode', 'ASSIGN')
    ...
    def initialize_goals(self):
            # 목적지를 군집화
            kmeans = KMeans(n_clusters=self.numAgent)
            kmeans.fit(self.agent_goals)
            clusters = {i: [] for i in range(kmeans.n_clusters)}
            
            for idx, label in enumerate(kmeans.labels_):
                clusters[label].append(self.agent_goals[idx])

            # 군집화된 목적지를 각 에이전트에 할당
            for i in range(self.numAgent):
                agent_id = f'Agent_{i}_Planner'
                self.agents_goals[agent_id] = clusters[i] if i in clusters else []
