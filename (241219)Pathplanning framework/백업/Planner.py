...
from Models.Atomic.GPP  import RRT
...
class Planner(DEVSAtomicModel):
    def __init__(self, ID, objConfiguration):
        ...
        self.target_tolerance = 1 * self.robot_radius
        lstTeamComposition = [[ID]]
        # self.objAIPlanner = objAIPlanner

        self.setup_plot()  # plot 설정을 위한 메소드 호출
        ...
        self.addStateVariable("target_yaw",0)

    def setup_plot(self):
        # Matplotlib 그래프 설정
        self.figure, self.ax = plt.subplots()
        # ax 속성을 저장해서 다른 메소드에서 사용할 수 있도록 함
        self.ax.set_xlim(-100, 100)  # X축의 범위 설정
        self.ax.set_ylim(-100, 100)  # Y축의 범위 설정
    ...
    def funcOutput(self):
        if self.getStateValue('mode') == "LPP":
            objRequestMessage = MsgManeuverState(self.ID, 
                                                          self.getStateValue('target_x'), 
                                                          self.getStateValue('target_y'),
                                                          self.getStateValue('target_yaw'))
    ...
    def plot_trajectory(self, trajectory):
        if 'planner' not in self.lines:
            self.lines['planner'], = self.ax.plot([], [], 'ro-', label='planner', linewidth=1, markersize=4)
        
        # trajectory에서 x, y 데이터를 받아서 사용
        xdata = trajectory[:, 0]
        ydata = trajectory[:, 1]

        # 받은 데이터를 line 객체에 설정
        self.lines['planner'].set_data(xdata, ydata)

        # 축을 재설정하고 자동 스케일링을 통해 모든 데이터 포인트가 보이도록 조정
        self.ax.relim()
        self.ax.autoscale_view()

        # 그래프를 다시 그리고 잠시 멈춤으로써 UI를 업데이트
        plt.draw()
        plt.pause(0.01)
    ...
    #         plt.title('Computed Trajectory for LPP')
    #         plt.legend()
    #         plt.grid(True)
    #         plt.show()
    ...
    def funcInternalTransition(self):
        if self.getStateValue('mode') == 'LPP':
            ...
            if dist_to_goal <= self.target_tolerance*1:
                print("Goal reached.")
                ...
                if path_index < len(self.path):
                    ...
                    self.curPose = trajectory[-1].tolist()
           ...
           self.setStateValue("mode","WAIT")
            ...
            # raw_path = RRT(tuple([starts_x, starts_y]), tuple(self.goal), self.terrain_polygons)
            # Simplify the path using line of sight
            # self.path = simplify_path_with_los(raw_path, self.terrain_polygons) if raw_path else []
            # self.path = self.path[1:]#다음 경로부터
            # plot_path_with_terrain(self.starts[self.id], self.goal , self.path, self.terrain_polygons)
            # self.setStateValue("path_index", 0)
    ...
    def funcTimeAdvance(self):
        if self.getStateValue('mode') == "LPP":
            # return float(self.predict_time)/10
            return 5
